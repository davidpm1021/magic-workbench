...
`kind`, when present, names the list: spell modes, optional costs, a number,
a counter type, a pile. On `optionalCost` lists each option carries its `cost`
and whether the player can `afford` it now. Engines that do not know leave
them out.
